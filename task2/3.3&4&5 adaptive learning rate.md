...
- AdaGrad(Adaptive Gradient), the larger gradient, the smaller learning rate.
  ...
  - $$\theta_{3}^{i} \leftarrow \theta_{2}^{i}-\frac{\eta}{\sigma_{2}^{i}} g_{2}^{i} \quad \sigma_{2}^{i}=\sqrt{\frac{1}{3}\left[\left({g}_{0}^{i}\right)^{2}+\left({g}_{1}^{i}\right)^{2}+\left({g}_{2}^{i}\right)^{2}\right]}$$
